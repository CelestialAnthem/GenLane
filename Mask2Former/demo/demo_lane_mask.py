...
if __name__ == "__main__":
    ...
    if args.input:
        if len(args.input) == 1:
            if '.txt' in args.input[0]:  # jsons in txt
        ...
        for path in tqdm.tqdm(args.input, disable=not args.output):
            ...
            if args.output:
                ...
                cv2.imwrite(out_filename,visualized_output*255)
                
                # img_arr = np.zeros_like(img)
                # img_arr[...] = np.expand_dims(visualized_output, axis=-1)
                # merge_output = cv2.addWeighted(img, 0.5, img_arr*255, 0.5, 0)
                # cv2.imwrite(out_filename.replace('.jpg','_m.jpg'),merge_output)          
            else:
