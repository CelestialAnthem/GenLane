...
id_to_color = {item['id']: item['color'] for item in MAPILLARY_VISTAS_SEM_SEG_CATEGORIES}
print(id_to_color)
...
class VisualizationDemo(object):
    ...
    def draw_sem_seg_pure(self, image):
        # id_to_color = {item['id']: item['color'] for item in MAPILLARY_VISTAS_SEM_SEG_CATEGORIES}
        ...
        for i in range(height):
            for j in range(width):
                new_image[i, j] = id_to_color[image[i, j].item()]
        import matplotlib.pyplot as plt
        plt.imsave('image_matplotlib.png', new_image)
        
        from PIL import Image
        array = new_image.astype('uint8')
        haha_ = Image.fromarray(array)
        haha_.save('image_pillow.png')
        return VisImage(new_image)
    
    
    def run_on_image(self, image, pure=False):
        """
        Args:
        ...
        return predictions, vis_output, pred, res

    def run_lane_binary_mask_on_image(self, image):
        """
        Args:
            image (np.ndarray): an image of shape (H, W, C) (in BGR order).
                This is the format used by OpenCV.
        ...
        binary_mask = np.zeros((sem_seg.shape[0],sem_seg.shape[1]))
        for label in filter(lambda l: l < len(self.metadata.stuff_classes), labels):
            text = self.metadata.stuff_classes[label]
            # if text in ["Lane Marking - Crosswalk","Lane Marking - General", "Bike Lane", "Curb Cut", "Curb"]:
            if text in ["Lane Marking - General", "Bike Lane"]:
                binary_mask += (sem_seg == label).astype(np.uint8)
            else:
                continue
            
        return predictions, binary_mask
