...
SAVE_DIR="/root/GenLane/output_x/"
...
inf() {
    ...
    INPUT_FILE_LIST="/mnt/share_disk/songyuhao/seg_cleanlab/test/$index/$gpu_id.txt"
...
cleanlab(){
    ...
    for pred_file in $SAVE_DIR/comb/$index/pred*.npy; do
        ...
        if [[ -f "$res_file" ]]; then
            ...
            python /root/cleanlab/cleanlab/run.py "$pred_file" "$res_file" "$base" "$SAVE_DIR/clean_res/$index/" $index True 0.1
        else
...
do
    ...
    wait
    cleanlab $i
    cp /root/GenLane/output_x/clean_res/$i/$i.json /mnt/ve_share/songyuhao/seg_cleanlab
done
