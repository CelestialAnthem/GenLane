...
class IMGBlurProcessor():
    def __init__(self, input: list or str, strength: int, max_iter: int, output: str ="", blur_type: str = "Gaussian") -> None:
        self.input = input
        self.output = output
    ...
    def input_reader(self) -> None:
        ...
        self.lane_mask_calculator = LaneMaskCalculator(self.strength, self.amount)
    ...
    def blur_img_processor(self, imgs: list, blur_type: str, p1: list = [], p2: list = [], p3: list = []) -> list:
        ...
        p2 = [0] * self.amount if p2 == [] else p2
    ...
    def bila_calculator(self, strengths: list):
        bila_lst_1 = [5 + int((math.sqrt(strength) / math.sqrt(0.5)) * 4) if strength > 0 else 5 for strength in strengths]
        bila_lst_2 = [75 + int((math.sqrt(strength) / math.sqrt(0.5)) * 75) if strength > 0 else 75 for strength in strengths]
    ...
    def run(self,) -> None:
        start = datetime.now()
        self.input_reader()
        for i in range((len(self.input_imgs) // 101) + 1):
            ...
            ori_paras = [_ * 100 if _ > 0 else 10 for _ in strengths]
            cycle_paras = [_ * 20 if _ > 0 else 2 for _ in strengths]
...
if __name__ == "__main__":
    ...
    input = "/data_path/100_lane_day.txt"
    strength = 0.3
    max_iter = 5
    # input = "/share/2d-od/lei/data/lane_700"
    output = "/share/2d-od/lei/data/test_output" 
    # % blur_type
    img_blur_processor = IMGBlurProcessor(input, strength, max_iter, output, blur_type)
