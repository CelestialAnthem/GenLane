...
class IMGBlurProcessor():
    def __init__(self, input: list or str, target_amb: int, output: str ="", blur_type: str = "Gaussian") -> None:
        ...
        if self.output == "" and type(self.input) == str:
            self.output = "%s_%s_v1.3" % (self.input, self.blur_type)
    ...
    def blur_img_processor(self, imgs: list, blur_type: str, p1: list = [], p2: int = 0, p3: int = 0) -> list:
        
        p1 = [0] * self.amount if p1 == [] else p1
        ...
        def worker(_):
            ...
            if blur_type == "Gaussian":
                ...
                temp_img = cv2.bilateralFilter(img, d=5, sigmaColor=120, sigmaSpace=75)
        ...
        combine_lst = list(map(lambda a, b: [a, b], imgs, p1))
    ...
    def cycle_post_process(self, solid_masks: list, img_paths: list, imgs: list, iterations: int, para_list: list):
    ...
    def para_calculator(self, ori_paras: list, cycle_paras: list, max_iter : int) -> list:
        para_lst = []
        for i in range(max_iter):
            iter_lst = [ori_paras[_] + (i + 1) * cycle_paras[_] for _ in range(self.amount)]
    ...
    def run(self,) -> None:
        ...
        for i in range((len(self.input_imgs) // 100) + 1):
            ...
            strength = [self.target_amb - _ for _ in ambiguitys]
            ori_paras = [_ * 100 if _ > 0 else 10 for _ in strength ]
            cycle_paras = [_ * 20 if _ > 0 else 2 for _ in strength ]
            ...
            # Post-process
            max_iter = 5
            cycle_paras = self.para_calculator(ori_paras, cycle_paras, max_iter)
            print(cycle_paras)
            cycle_imgs = self.cycle_post_process(solid_masks, res_img_paths, replaced_imgs, max_iter, cycle_paras)
